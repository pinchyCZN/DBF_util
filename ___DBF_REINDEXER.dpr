...
function check_str_bool(s:string):boolean;
begin
  result:=false;
  if(AnsiPos('1',s)<>0)then
    result:=True;
  if(AnsiPos('Y',s)<>0)then
    result:=True;
  if(AnsiPos('y',s)<>0)then
    result:=True;
  if(AnsiPos('T',s)<>0)then
    result:=True;
  if(AnsiPos('t',s)<>0)then
    result:=True;
end;
function get_ini_params(var pack,zap,usebde:boolean;var fname:string):Boolean;
var ini:Tinifile;
s:string;
begin
 s:=ChangeFileExt(ParamStr(0),'.ini');
 ini:=TIniFile.Create(s);
 s:=ini.ReadString('DBF','FNAME','');
 if(s<>'')then
  fname:=s;
 s:=ini.ReadString('DBF','PACK','');
 if(s<>'') and (check_str_bool(s)) then
  pack:=true;
 s:=ini.ReadString('DBF','ZAP','');
 if(s<>'') and (check_str_bool(s)) then
  zap:=true;
 s:=ini.ReadString('DBF','BDE','');
 if(s<>'')then
 begin
  usebde:=check_str_bool(s)
 end;
 FreeAndNil(ini);
end;
var
...
begin
  ...
  zap:=false;
  usebde:=true;
  get_ini_params(pack,zap,usebde,fname);
...
//     Writeln('Parameter '+IntToStr(i)+' = '+ParamStr(i));
  ...
  begin
    Writeln('params -pack -zap -bde -apollo');
    Writeln('INI file:'+ChangeFileExt(ParamStr(0),'.ini'));
    Writeln('[DBF] FNAME,PACK,ZAP,BDE');
    if(nocmd)then
    begin
